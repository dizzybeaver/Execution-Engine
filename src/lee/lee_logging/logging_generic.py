...
# Import debug config for SUGA-ISP compliance
import re
import traceback

from lee.lee_logging.logging_manager import get_logging_core
from lee.lee_logging.logging_types import ErrorLogLevel
...
try:
    ...
    def get_debug_config():
        """Fallback debug config that returns empty config."""
        class EmptyConfig:
            def is_debug_enabled(self, scope):  # pylint: disable=unused-argument
                return False
        return EmptyConfig()
...
def _sanitize_exception_details(error: str | Exception,
        ...
        try:
            ...
            if tb_lines:
                ...
                if match:
                    ...
                    result += f"\n  at {filename}:{lineno}"
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError, OSError):
...
def _execute_log_error_implementation(message: str, error: str | Exception = None, **kwargs) -> None:
    ...
    if error:
        ...
        try:
            # FIXED: SUGA-ISP compliance - Removed direct lee_debug import get_debug_config
    ...
    # FIXED: Don't pass error explicitly - it's already in kwargs if provided
    # This prevents "got multiple values for keyword argument 'error'" error
    core.log_error_with_tracking(message, level=level, **kwargs)

def _execute_log_debug_implementation(message: str, **kwargs) -> None:
    """Log debug message (only if LEE_DEBUG enabled)."""
...
def _execute_log_operation_failure_implementation(operation_name: str, error: str | Exception, **kwargs) -> None:
...
def _execute_log_reset_implementation(**kwargs) -> bool:  # pylint: disable=unused-argument
