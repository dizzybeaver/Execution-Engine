...
# SUGA-ISP Gateway imports (compliant)
...
_DEBUG_TIMINGS_ENABLED = os.getenv("DEBUG_TIMINGS", "false").lower() == "true"
...
def _is_debug_timings() -> bool:
    """Check if DEBUG_TIMINGS is enabled (cached value)."""
    return _DEBUG_TIMINGS_ENABLED


def _print_debug(msg: str):
    """Print debug message only if LEE_DEBUG=true - SUGA-ISP compliant."""
    if _is_debug_mode():
        try:
            from lee.gateway import GatewayInterface, execute_operation
            execute_operation(GatewayInterface.DEBUG, "log",
                             corr_id=f"ssm_{int(time.time() * 1000)}",
                             scope="SSM",
                             message=f"[SSM_DEBUG] {msg}")
...
def _print_timing(msg: str):
    """Print timing message only if DEBUG_TIMINGS=true - SUGA-ISP compliant."""
    if _is_debug_timings():
        try:
            execute_operation(GatewayInterface.DEBUG, "log",
                             corr_id=f"ssm_{int(time.time() * 1000)}",
                             scope="SSM",
                             message=f"[SSM_TIMING] {msg}")
        except ImportError as import_error:
            try:
                execute_operation(
                    GatewayInterface.LOGGING,
...
try:
    ...
    _CACHE_TTL = 300  # Fallback to default if invalid
...
def _get_ssm_client():
    ...
    try:
        ...
        try:
            ...
            from lee.lambda_preload import _BOTO3_SSM_CLIENT
            ...
            _print_debug(f"lambda_preload not available ({import_error}), will create fresh client")
    ...
    except (ConnectionError, TimeoutError, RuntimeError, ImportError, ValueError, TypeError, KeyError, AttributeError, OSError) as init_error:
        _init_time = (time.perf_counter() - _init_start) * 1000
        _print_timing(f"!!! SSM client initialization FAILED: {_init_time:.2f}ms")
        _print_debug(f"SSM client initialization failed: {init_error}")
        # SUGA-ISP compliant error logging via Gateway
        try:
            corr_id = generate_correlation_id("cfg")
            execute_operation(GatewayInterface.LOGGING, "log_error",
                             corr_id=corr_id, scope="CONFIG",
                             message=f"Failed to initialize SSM client: {init_error}")
        except ImportError:
...
def get_ha_token(use_cache: bool = True) -> Optional[str]:
    ...
    if use_cache:
        ...
        # SUGA-ISP compliant cache operation via Gateway
        try:
    ...
    try:
        ...
        # SUGA-ISP compliant info logging via Gateway
        try:
            corr_id = generate_correlation_id("cfg")
            execute_operation(GatewayInterface.LOGGING, "log_info",
                             corr_id=corr_id, scope="CONFIG",
                             message=f"[SSM GET] Token retrieved successfully (length={len(token)})")
        except ImportError:
            # Optional dependency - continue if unavailable
            ...
        ...
        if use_cache:
            ...
            # SUGA-ISP compliant cache operation via Gateway
            try:
                execute_operation(GatewayInterface.CACHE, "set",
                                 key=cache_key, value=token, ttl=_CACHE_TTL)
            except ImportError:
                # Optional dependency - continue if unavailable
                ...
    ...
    except (ConnectionError, TimeoutError, RuntimeError, KeyError, ValueError, TypeError, AttributeError, OSError) as token_error:
        _error_time = (time.perf_counter() - _op_start) * 1000
        _print_timing(f"!!! EXCEPTION after {_error_time:.2f}ms: {token_error}")
        _print_debug(f"Exception retrieving token: {token_error}")
        # SUGA-ISP compliant error logging via Gateway
        try:
            corr_id = generate_correlation_id("cfg")
            execute_operation(GatewayInterface.LOGGING, "log_error",
                             corr_id=corr_id, scope="CONFIG",
                             message=f"[SSM GET] Token retrieval failed: {token_error}")
        except ImportError:
            # Optional dependency - continue if unavailable
            pass
        ...
        if use_cache:
            # SUGA-ISP compliant cache operation via Gateway
            try:
                execute_operation(GatewayInterface.CACHE, "delete", key=cache_key)
            except ImportError:
                # Optional dependency - continue if unavailable
                ...
...
def invalidate_token_cache():
    ...
    # SUGA-ISP compliant cache and logging operations via Gateway
    try:
        corr_id = generate_correlation_id("cfg")
        execute_operation(GatewayInterface.CACHE, "delete", key="ssm_ha_token")
        execute_operation(GatewayInterface.LOGGING, "log_info",
                         corr_id=corr_id, scope="CONFIG",
                         message="[SSM CACHE] Token cache invalidated")
    except ImportError:
        # Optional dependency - continue if unavailable
        ...
