...
- Factory contains actual implementation (DI container)
"""

from typing import Any, Dict, Optional, Callable, Type
import logging

from EE.foundation.di.di_factory import DIFactory


class DIInterface:
    """DI Interface - Routes dependency injection operations.
    ...
    def __init__(
        ...
        self._logger = self._get_logger("foundation.di")
        self._metrics = self._get_metrics("foundation.di")

        # Factory pool - reuses factory instances
        self._factory_pool: Dict[int, DIFactory] = {}
        self._pool_lock = __import__("threading").RLock()

    def execute_operation(self, operation: str, **kwargs) -> Any:
        """
        Execute DI operation (Interface Method).
    ...
    def _get_factory(self) -> DIFactory:
        ...
        with self._pool_lock:
            if thread_id not in self._factory_pool:
                self._factory_pool[thread_id] = DIFactory(
                    logger=self._logger,
    ...
    def _return_factory(self, factory: DIFactory) -> None:
