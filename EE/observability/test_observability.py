...
# Add EE src to path FIRST
script_dir = Path(__file__).parent
project_root = script_dir.parent.parent
ee_src = project_root / "EE" / "src"
ee_root = project_root / "EE"

sys.path.insert(0, str(ee_src))
sys.path.insert(0, str(ee_root))

# Now import with correct module names
...
def test_ug_compliance_check():
    ...
    for file in required_files:
        if file.exists():
            ...
            print(f"  ✗ MISSING: {file.relative_to(script_dir)}")
